...
export default function Forecast({
  ...
  if (!loading) {
    return (
      <div
        className={clsx(
          "w-[96vw] md:w-[65vw] h-full bg-zinc-800 p-2 absolute top-0 left-0 rounded-md grid grid-rows-[30px_auto]  justify-items-center z-[10] overflow-auto text-zinc-300 overflow-auto font-semibold",
          {
            grid: open,
            hidden: !open,
          }
          ...
          {type === "taf"
            ? data.map((info, i) => {
                ...
                startNew = `${startNew.monthShort} ${startNew.day}, ${String(
                  startNew.hour
                ).padStart(2, "0")}:${String(startNew.minute).padStart(
                  2,
                  "0"
                )}`;
                ...
                return (
                  <div
                    key={i}
                    className="grid items-center justify-items-center relative text-zinc-300 h-[20vh] w-[85vw] md:w-[50vw] overflow-auto bg-zinc-900 rounded-lg p-2 mt-4 md:mt-0"
                  >
