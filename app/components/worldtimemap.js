...
export default function Map({ airports, wind }) {
    ...
    wind.forEach((item) => {
      ...
      if (
        ...
        const marker1 = new maptilersdk.Marker({ element: pin })
          ...
          .setRotation(
            +item[alt - (9 - (item.length - 3))].slice(0, 2) * 10 + 90
          )
          .addTo(map.current);
  ...
  return (
    <div className="grid items-center justify-items-center">
      <div className="flex gap-[10%] absolute top-[13%] left-[15%] md:left-[35%] w-[70%] md:w-[40%] justify-content-center">
        <select
          onChange={(e) => setAlt(e.target.value)}
          className="bg-neutral-300 text-neutral-800 rounded-lg w-[45%] md:w-[30%] h-[25px]"
        >
      ...
      <div
        ref={mapContainer}
        className="rounded-lg mt-[8vh] w-[90vw] md:w-[70vw] h-[30vh]"
      />
    </div>
  );
}
