...
export default function Runways({ airportCode, stored, coords }) {
  ...
  useEffect(() => {
    ...
    let varData = model.point([coords.split(",")[0], coords.split(",")[1]]);
    varData = varData.decl ? varData.decl : 0;
    setMagVar(varData);
  }, [airportCode, stored, coords]);

  let windDir,
    windSpeed = 0;
  if (windData.length !== 0) {
    windSpeed = windData[2] ? windData[2]["value"] : windData[0]["value"];
    windDir = windData[1] ? windData[1]["value"] + magVar : 0 + magVar;
  }
                ...
                <div className="h-10 min-w-full">
                  <div className="h-full w-full bg-zinc-800 rounded px-1 flex justify-between items-center">
                    <div className="h-10 w-6 grid gap-1 items-center">
                      {[1, 2, 3, 4].map((line) => {
                        return (
                          <div key={line} className="h-1 w-4 bg-zinc-300"></div>
                        );
                      })}
                    </div>
                    ...
                    <div className="h-10 w-6 grid gap-1 items-center">
                      {[1, 2, 3, 4].map((line) => {
                        return (
                          <div key={line} className="h-1 w-4 bg-zinc-100"></div>
                        );
                      })}
                    </div>
                  </div>
