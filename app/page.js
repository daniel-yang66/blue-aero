...
export default async function Home({ searchParams }) {
  ...
  let diagram;
  if (airportCode) {
    diagram = await AirportDiagram(airportCode.split("/")[0]);
  }
  ...
  if (airportCode) {
    return (
      <div className=" mt-[1vh] max-h-[80vh] w-full grid grid-rows-[30vh_3vh_1fr] md:grid-rows-[33vh_4vh_auto] gap-[1vh] justify-items-center">
        <Suspense fallback={<Loading />}>
          <CurrentConditions
            airportCode={airportCode.split("/")[0]}
            stored={stored}
          />
        </Suspense>
        <div className="flex gap-2">
          <Link
            ...
            href={`/flights${airportCode ? `?airportCode=${airportCode.split("/")[0]}` : ""}`}
          >
            {airportCode.split("/")[0]} Flights
          </Link>
    ...
    return (
      <h1 className="text-xl md:text-2xl text-zinc-300 font-bold mt-[25vh]">
        Weather Status One Search Away
      </h1>
    );
  }
}
